/**
 * adapted from smlng PINGPONG example
 *
 * 
 */
// standard
...
int main(void)
{
	...
	if(sw_network_init() < 0)
	{
		puts("Error initializing network interface");
		exit(-1);
	}
	
	// start data collection (from sensors) thread(s)
...
// for use from command line, reuses pp_send method
// returns 0 on succes, 1 on failure (see pp_send)
static int sw_send(int argc, char **argv)
{
	/*
	...
	*/
	...
	if(argc != 3)
	{
		puts("usage: send dst_addr data");
		return 1;
	}
	
	return pp_send(argv[1], argv[2]);
}
...
static void *_receiver(void *arg)
{
    ...
    while (1) {
        ...
        if ((res = recvfrom(pp_socket, pp_buffer, sizeof(pp_buffer), 0,
        	...
        	// check for PING or PONG or SW_PING or SW_PONG
            ...
            else if (strcmp(pp_buffer, SW_PING) == 0) {
                printf(". received SW_PING from [%s].\n", src_addr_str);
                pp_send(src_addr_str, SW_PONG);
            }
            else if (strcmp(pp_buffer, SW_PONG) == 0) {
                printf(". received SW_PONG from [%s].\n", src_addr_str);
            }
            else {
                printf(". received data from [%s] with %d bytes.\n", src_addr_str, res);
                printf("received message: %s\n", pp_buffer);
                
                // execute function to read message, send back data etc...
